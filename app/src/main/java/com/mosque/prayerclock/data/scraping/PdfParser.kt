...
class PdfParser @Inject constructor() {
    ...
    private fun extractDistricts(text: String): List<String> {
        ...
        for (pattern in districtPatterns) {
            ...
            if (match != null) {
                ...
                val foundDistricts = districtsText
                    .replace("-", ",")
                    .split(",")
    ...
    private fun extractMonthAndYear(text: String): Pair<String, Int> {
        ...
        val month = if (dateMatches.isNotEmpty()) {
    ...
    private fun extractPrayerTimes(text: String, year: Int): List<DailyPrayerTime> {
    ...
    private fun convertTo24Hour(time: String, period: String): String {
        return when {
    ...
    fun convertToPrayerTimesEntities(
        ...
        zone: Int,
        iqamahGaps: Map<String, Int> = emptyMap() // Default iqamah gaps in minutes
    ): List<PrayerTimes> {
        val providerKey = "ACJU_DIRECT:$zone"
        val defaultGaps = mapOf(
            "fajr" to 15,
            "dhuhr" to 15,
            "asr" to 15,
            "maghrib" to 5,
        ...
        return prayerTimesData.prayerTimes.map { dailyTime ->
            ...
            val fajrIqamah = addMinutesToTime(dailyTime.fajr, iqamahGaps["fajr"] ?: defaultGaps["fajr"]!!)
            val dhuhrIqamah = addMinutesToTime(dailyTime.dhuhr, iqamahGaps["dhuhr"] ?: defaultGaps["dhuhr"]!!)
            val asrIqamah = addMinutesToTime(dailyTime.asr, iqamahGaps["asr"] ?: defaultGaps["asr"]!!)
            val maghribIqamah = addMinutesToTime(dailyTime.maghrib, iqamahGaps["maghrib"] ?: defaultGaps["maghrib"]!!)
            val ishaIqamah = addMinutesToTime(dailyTime.isha, iqamahGaps["isha"] ?: defaultGaps["isha"]!!)
